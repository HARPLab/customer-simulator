...
Restaurant simulation in VREP through the Python Remote API
...
2. Copy the files vrep.py, vrep.pyc, vrepConst.py, vrepConst.pyc, remoteApi.so, main.py and the models folder from the Restaurant Simulation folder from this git into the desired project folder

3. Change the address variable to point to where the models folder is saved.

# Restaurant Simulation:
 The restaurant simulation can now be run by running the main.py file and it will generate a randomized restaurant environment inside the VREP scene remotely from the python script. 
 ...
 3. Markup the joints of the people with actual pose data from a restaurant
